...
main()
{
    Application::Get().SetProcessHandle(GetModuleHandle(NULL));
    
	Application::Get().OnInitialize.AddListener<&InitEditor>();
	Application::Get().OnRender.AddListener<&RenderEditor>();
	Application::Get().Run();
}
```
