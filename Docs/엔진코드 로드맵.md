...
- 씬을 저장할 때 Serializer와 GUID Helper 이용하여 Version과 GUID 시드, 각 오브젝트 별 GUID를 생성 저장한다.
...
- 모델 파일을 불러오면 Prefab으로 전환 시켜주는데, 컴포넌트에 들어가는 리소스로서 부착되는 형태이면 자동으로 리소스화 시켜줌 (Asset/Resource/[ Mesh, Texture , Material, AnimationClip ]에 Json파일로 만들어 줌)
...
- 기본적으로 아래와 같은 순서로 렌더패스가 구성됨
  - 포워드
    - 쉐도우맵
    - 스카이박스
    - Opaque 및 Transparent ( 머터리얼 렌더패스 )
