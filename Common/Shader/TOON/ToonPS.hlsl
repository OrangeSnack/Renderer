...
Texture2D _roughness : register(t20);
Texture2D _ambientOcclusion : register(t21);
...
cbuffer ToonMatBuffer : register(b1)
{
    float4 mBaseColor;
...
float CalculateShadowPCF(float4 LightPos)
{
    ...
    for (int x = -1; x <= 1; ++x)
    {
        for (int y = -1; y <= 1; ++y)
        {
            ...
            shadow += _shadowmap.SampleCmpLevelZero(_cmpsp0, texCoords + offset, currentDepth);
        }
    }
    shadow /= 9.0f;
